...
"""Pydantic schemas for tax calculation requests and responses."""
...
class EmploymentInputSchema(BaseModel):
    gross_salary: float = Field(0.0, ge=0, description="Annual gross salary (€)")
...
class DeductionsInputSchema(BaseModel):
    commute_km: float = Field(
        0.0, ge=0, le=500, description="One-way commute distance in km"
    )
    commute_days: int = Field(0, ge=0, le=366)
    home_office_days: int = Field(0, ge=0, le=366)
