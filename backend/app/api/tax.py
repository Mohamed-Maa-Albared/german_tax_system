...
def calculate_tax(request: TaxCalculationRequest, db: Session = Depends(get_db)):
    """Calculate German income tax for given inputs using the active year's parameters."""
    params = parameter_service.get_parameters_by_year(db, request.personal.tax_year)
    if not params:
        params = parameter_service.get_active_parameters(db)
    if not params:
        raise HTTPException(status_code=503, detail="Tax parameters not configured.")
    ...
    inp = TaxCalculationInput(
        personal=PersonalInput(
            ...
            occupation_type=request.personal.occupation_type,
        ),
        ...
        deductions=DeductionsInput(
            ...
            home_office_type=request.deductions.home_office_type,
            arbeitszimmer_mittelpunkt=request.deductions.arbeitszimmer_mittelpunkt,
            apartment_sqm=request.deductions.apartment_sqm,
            office_sqm=request.deductions.office_sqm,
            monthly_warm_rent=request.deductions.monthly_warm_rent,
            your_rent_share_pct=request.deductions.your_rent_share_pct,
            arbeitszimmer_start_month=request.deductions.arbeitszimmer_start_month,
            teacher_materials=request.deductions.teacher_materials,
            double_household_costs_per_month=request.deductions.double_household_costs_per_month,
            double_household_months=request.deductions.double_household_months,
        ),
